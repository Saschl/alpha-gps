...
class SonyContentCatalogTest {
    ...
    fun versionFourUsesContentCommandsAndVendorEventBeforeListingAndDownload() = runTest {
        ...
        assertEquals(200, initialPage.totalObjects)
        assertTrue(initialPage.hasMore)
        val lastPage = transfer.page(200)
        assertEquals(202, lastPage.totalObjects)
        assertEquals(2, lastPage.photos.size)
        assertEquals(101L, lastPage.photos.first().handle and 0xffffffffL)
        assertEquals(listOf(1099L, 0L, 100L, 1L, 0L), requests.last().second)
